...
<form method="post" action="<?= $this->url->href('ProjectEditController', 'update', array('project_id' => $project['id'], 'redirect' => 'edit')) ?>" autocomplete="off">
    ...
    <fieldset>
        ...
        <?php /* <?= $this->form->label(t('Email'), 'email') ?>
        <?= $this->form->email('email', $values, $errors, array('tabindex="2"', 'autocomplete="email"')) ?> */ ?>
        <?php /* <p class="form-help"><?= t('The project email is optional and could be used by several plugins.') ?></p> */ ?>

        <?php /*  <?= $this->form->label(t('Identifier'), 'identifier') ?>
        <?= $this->form->text('identifier', $values, $errors, array('maxlength="50"', 'tabindex="3"')) ?> */ ?>
        <?php /* <p class="form-help"><?= t('The project identifier is optional and must be alphanumeric, example: MYPROJECT.') ?></p> */ ?>
        ...
        <?php /*  <?= $this->form->checkbox('per_swimlane_task_limits', t('Task limits apply to each swimlane individually'), 1, $project['per_swimlane_task_limits'] == 1, '', array('tabindex' => 5)) ?> */ ?>
    ...
    <fieldset>
        ...
        <?php if ($this->app->config('disable_private_project') != 1): ?>
            <?php if ($this->user->hasProjectAccess('ProjectCreationController', 'create', $project['id'])): ?>
                ...
                <?php /*   <p class="form-help"><?= t('Personal projects do not have users and groups management.') ?></p> */ ?>
            <?php endif ?>
        <?php endif ?>
