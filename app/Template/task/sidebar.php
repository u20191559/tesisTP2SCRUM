<div class="sidebar sidebar-icons">
    ...
    <ul>
        ...
        <?php /* <li <?= $this->app->checkMenuSelection('ActivityController', 'task') ?>>
            <?= $this->url->icon('dashboard', t('Activity stream'), 'ActivityController', 'task', array('task_id' => $task['id'])) ?>
        </li> */ ?>
        <?php /* <li <?= $this->app->checkMenuSelection('TaskViewController', 'transitions') ?>>
    ...
    <ul>
        ...
        <li>
        <?php // <?= $this->modal->medium('refresh fa-rotate-90', t('Edit recurrence'), 'TaskRecurrenceController', 'edit', array('task_id' => $task['id'])) ?>
        </li>
        <?php endif ?>
        <?php /*  <li>
          <?php /*  <?= $this->modal->medium('plus', t('Add a sub-task'), 'SubtaskController', 'create', array('task_id' => $task['id'])) ?>
        </li> */ ?>
        <?= $this->hook->render('template:task:sidebar:after-basic-actions', array('task' => $task)) ?>

        <li>
            <?php // <?= $this->modal->medium('code-fork', t('Add internal link'), 'TaskInternalLinkController', 'create', array('task_id' => $task['id'])) ?>
        </li>
        <li>
            <?php // <?= $this->modal->medium('external-link', t('Add external link'), 'TaskExternalLinkController', 'find', array('task_id' => $task['id'])) ?>
        ...
        <li>
        <?php // <?= $this->modal->medium('camera', t('Add a screenshot'), 'TaskFileController', 'screenshot', array('task_id' => $task['id'])) ?>

        </li>
        <?= $this->hook->render('template:task:sidebar:after-add-attachments', array('task' => $task)) ?>

        <li>
            <?php // <?= $this->modal->small('files-o', t('Duplicate'), 'TaskDuplicationController', 'duplicate', array('task_id' => $task['id'])) ?>
        ...
        <li>
        <?php // <?= $this->modal->small('clone', t('Move to project'), 'TaskDuplicationController', 'move', array('task_id' => $task['id'], 'project_id' => $task['project_id'])) ?>
        </li>
        <?php /*
        <li>
        ...
        <?php /* <?php if ($task['is_active'] == 1 && $this->projectRole->isSortableColumn($task['project_id'], $task['column_id'])): ?>
            <li>
